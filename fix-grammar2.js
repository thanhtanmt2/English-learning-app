...
function getRandomMeanings(exclude, count = 3) {
  const allMeanings = Object.values(vieMap);
  const shuffled = allMeanings.sort(() => 0.5 - Math.random());
  const result = [];
  for (const m of shuffled) {
    if (m !== exclude && !result.includes(m)) {
      result.push(m);
    }
...
function getRandomOptions(correctAnswer) {
  const wrongs = getRandomMeanings(correctAnswer, 3);
  const options = [correctAnswer, ...wrongs].sort(() => 0.5 - Math.random());
  const labels = ['A', 'B', 'C', 'D'];
  let correctLabel = '';
  const parts = [];
  options.forEach((opt, idx) => {
    parts.push(`${labels[idx]}: ${opt}`);
    if (opt === correctAnswer) {
      correctLabel = labels[idx];
    }
  });
  return { text: parts.join(' | '), correctLabel };
}

async function fixAllQuizQuestions() {
  ...
  console.log('Đang tạo lại quiz_question mới chuẩn...');
  
  const [rows] = await pool.query('SELECT id, word, meaning FROM words');
  
  let updated = 0;
  for (const row of rows) {
    const word = row.word;
    const meaning = row.meaning || vieMap[word.toLowerCase()] || 'không xác định';
    
    const { text: optionsText, correctLabel } = getRandomOptions(meaning);
    const newQuizQuestion = `Hỏi: '${word}' có nghĩa là gì? | ${optionsText} | Đáp án: ${correctLabel}`;
    
    await pool.execute('UPDATE words SET quiz_question = ? WHERE id = ?', [newQuizQuestion, row.id]);
    updated++;
  }

  console.log(`\n✅ Đã tạo lại ${updated} quiz questions thành công!`);
  process.exit(0);
}

fixAllQuizQuestions().catch(err => { console.error('Lỗi:', err.message); process.exit(1); });
